from tkinter import *



class LogInApp:
        def __init__(self, root):
                self.root = root
                self.root.geometry("800x500")

                self.createLogInTab()
        def createLogInTab(self):
                self.passwordEntry = StringVar()
                self.title = Label(self.root, text='Log in',font=(20)).pack(side="top",pady=50)
                self.entry1= Entry(self.root,font=(20)).pack(pady=40)
                self.entry2= Entry(self.root,font=20,show="*",textvariable=self.passwordEntry).pack()
                self.loginbutton = Button(self.root,text="LOG IN",command=self.GetLogInInfo).pack(pady=10)    
                self.registerbutton = Button(self.root,text="Click here to register!",command=self.OpenRegisterMenu).pack(side="bottom",pady=10)
        def GetLogInInfo(self):
                t2 = self.passwordEntry.get()
                print("password: ", t2)

        def OpenRegisterMenu(self):
if __name__ == "__main__":
    root = Tk()
    app = LogInApp(root)
    root.mainloop()